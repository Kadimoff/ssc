...
export function StartupCreatePage() {
  ...
  const create = useAction(() => apiClient.createStartup({
    slug: name.trim().toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, ''),

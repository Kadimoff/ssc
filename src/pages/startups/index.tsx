...
import type { OpenRole, StartupMembership } from '@/features/execution/types'
...
function TeamPanel({ slug, canManage }: { slug: string; canManage: boolean }) {
  ...
  const myMembership = state.selectedPersona === 'member' ? members.find((item) => item.userId === 'usr_10') : undefined
  return <div className='grid gap-6 lg:grid-cols-2'><Card><CardHeader><div className='flex items-center justify-between gap-2'><div><CardTitle>Core team</CardTitle><CardDescription>Named owners and current responsibilities.</CardDescription></div>{canManage && <MemberDialog slug={slug} />}</div></CardHeader><CardContent className='space-y-3'>{members.map((member) => <div key={member.id} className='flex min-h-14 items-center gap-3 rounded-xl border p-3'><span className='grid size-10 place-items-center rounded-xl bg-primary/10 font-semibold text-primary'>{member.name.split(' ').map((part) => part[0]).join('').slice(0, 2)}</span><div className='min-w-0 flex-1'><b className='block truncate text-sm'>{member.name}</b><p className='truncate text-xs text-muted-foreground'>{member.title} · {member.kind}</p></div>{canManage && <MemberEditDialog member={member} />}{canManage && member.kind !== 'founder' && <Button size='sm' variant='ghost' onClick={() => { store.removeMembership(member.id); toast.success('Member removed from demo team') }}>Remove</Button>}</div>)}{!members.length && <EmptyState title='No local team records' description='Invite the first member to this execution workspace.' icon={Users} />}{myMembership && <Button className='w-full' variant='outline' onClick={() => { store.removeMembership(myMembership.id); toast.success('You left the demo workspace') }}>Leave workspace</Button>}</CardContent></Card><Card><CardHeader><div className='flex items-center justify-between gap-2'><div><CardTitle>Open roles</CardTitle><CardDescription>Commitment is as important as craft.</CardDescription></div>{canManage && <RoleDialog slug={slug} />}</div></CardHeader><CardContent className='space-y-3'>{roles.map((role) => <div key={role.id} className='rounded-xl border p-4'><div className='flex justify-between gap-2'><div><b>{role.title}</b><p className='text-xs text-muted-foreground'>{role.department} · {role.commitment}</p></div><StatusBadge status={role.status} /></div><div className='mt-3 flex flex-wrap gap-1'>{role.skills.map((skill) => <Badge key={skill} variant='secondary'>{skill}</Badge>)}</div>{canManage && <div className='mt-3 flex flex-wrap gap-2'><Button size='sm' variant='outline' onClick={() => { store.updateRole(role.id, { status: role.status === 'open' ? 'paused' : 'open' }); toast.success('Role status updated') }}>{role.status === 'open' ? 'Pause role' : 'Reopen role'}</Button>{role.status !== 'filled' && <Button size='sm' variant='ghost' onClick={() => { store.updateRole(role.id, { status: 'filled' }); toast.success('Role closed') }}>Close role</Button>}</div>}</div>)}{!roles.length && <EmptyState title='No open roles' description='Add a specific need, expected commitment, and required skills.' icon={BriefcaseBusiness} />}</CardContent></Card></div>
}
...
function ActivityPanel({ slug }: { slug: string }) {
  ...
  return <Card><CardHeader><div className='flex flex-wrap items-start justify-between gap-3'><div><CardTitle>Startup activity</CardTitle><CardDescription>Workflow events, not popularity metrics.</CardDescription></div><Button variant='outline' asChild><a href={`${import.meta.env.BASE_URL}feed#feed-composer`}>Publish linked update</a></Button></div></CardHeader><CardContent className='space-y-1'>{rows.map((row) => <div key={row.id} className='grid gap-1 border-b py-3 text-sm sm:grid-cols-[minmax(0,1fr)_120px_180px]'><b>{row.title}</b><StatusBadge status={row.text} /><time className='text-muted-foreground'>{new Date(row.at).toLocaleString()}</time></div>)}{!rows.length && <EmptyState title='No operational activity yet' description='Milestone and workflow transitions will appear here.' icon={Activity} />}</CardContent></Card>
}
...
function MemberEditDialog({ member }: { member: StartupMembership }) {
  const { store } = useExecutionStore()
  const [title, setTitle] = useState(member.title)
  const [kind, setKind] = useState<StartupMembership['kind']>(member.kind)
  return <ResponsiveDialog title={`Edit ${member.name}`} description='Update the responsibility shown across this local demo workspace.' trigger={<Button size='sm' variant='outline'>Edit</Button>} footer={<Button disabled={title.trim().length < 2} onClick={() => { store.updateMembership(member.id, { title: title.trim(), kind }); toast.success('Team responsibility updated') }}>Save changes</Button>}><div className='grid gap-4'><Label>Responsibility<Input className='mt-2' value={title} onChange={(event) => setTitle(event.target.value)} /></Label><Label>Membership type<select className='mt-2 h-11 w-full rounded-lg border bg-background/55 px-3' value={kind} onChange={(event) => setKind(event.target.value as StartupMembership['kind'])}><option value='founder'>Founder</option><option value='member'>Member</option><option value='advisor'>Advisor</option></select></Label></div></ResponsiveDialog>
}

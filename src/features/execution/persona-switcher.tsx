...
export function PersonaSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return <ResponsiveDialog
    ...
    trigger={<Button variant='outline' size={compact ? 'icon' : 'sm'} className='gap-2' aria-label={`Demo persona: ${selected.label}`}><UsersRound />{!compact && <span className='max-w-40 truncate'>{selected.label}</span>}</Button>}
    footer={<div className='flex w-full items-center justify-between gap-3'><DemoDataBadge label='Demo only' /><span className='text-xs text-muted-foreground'>Selection persists in this browser.</span></div>}
  >
    <div className='grid gap-2'>
      {personaPresets.map((preset) => {
        ...
        return <button
          ...
          className={cn('flex min-h-14 items-center gap-3 rounded-xl border p-3 text-left transition-colors hover:border-primary/30 hover:bg-primary/[0.04] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary', active && 'border-primary/35 bg-primary/[0.07]')}
        >

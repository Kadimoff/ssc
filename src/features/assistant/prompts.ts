...
export function roleQuickPrompts(user?: User | null) {
  ...
  if (user && ['partner_admin', 'program_manager', 'platform_admin'].includes(user.activeRole)) return ['Find a university program partner', 'Show active joint programs', 'Find mentors for an MVP cohort']
